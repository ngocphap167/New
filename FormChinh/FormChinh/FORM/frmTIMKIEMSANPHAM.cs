...
namespace FormChinh.FORM
{
    public partial class frmTIMKIEMSANPHAM : Form
    {
       // DataTable tblTK;
        ...
        {
            string sql;
            sql = "SELECT * from tblHang where Mahang ='"+txtmasp.Text.Trim()+"'";
            DataGridView.DataSource = functions.GetDataToTable(sql);
            sql = "SELECT * from tblHang where Tenhang ='" + txttensp.Text.Trim() + "'";
            DataGridView.DataSource = functions.GetDataToTable(sql);
        }
        ...
        {
            string sql;
            sql = "SELECT * from tblHang where Mahang like '" + txtmasp.Text.Trim() + "%'";
            DataGridView.DataSource = functions.GetDataToTable(sql);
        }
        ...
        {
            string sql;
            sql = "SELECT * from tblHang where Tenhang like'" + txttensp.Text.Trim() + "%'";
            DataGridView.DataSource = functions.GetDataToTable(sql);
        }
        ...
        {
            string sql;
            sql = "select h.Mahang,Tenhang,maLoai,h.Soluong,Dongianhap,Dongiaban,Anh,Ghichu From tblChiTietHDNhap as ct, tblHDNhap as n, tblHang as h, NHACUNGCAP as ncc where ct.MaHDNhap = n.MaHDNhap and ct.Mahang = h.Mahang and ncc.maNCC = n.maNCC and ncc.maNCC like'" + txtNCC.Text.Trim() + "%'";
            DataGridView.DataSource = functions.GetDataToTable(sql);
        }

        private void txtNCC_MouseClick(object sender, MouseEventArgs e)
        {
            txtNCC.Clear();
        }
        /*private void excel()//DataGridView g; string sql; string tentap;
        {
            ...
            {
                obj.Cells[1, i] = DataGridView.Columns[i - 1].HeaderText;
            }
            for (int i = 0; i < DataGridView.Rows.Count; i++)
            {
                for (int j = 0; j < DataGridView.Columns.Count; j++)
                {
                    ...
                    {
                        obj.Cells[i + 2, j + 1] = DataGridView.Rows[i].Cells[j].Value.ToString();
                    }
                }
            }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            obj.Range["B4:D4"].Value = "DANH SÁCH SẢN PHẨM";
            for (int i = 1; i < DataGridView.Columns.Count + 1; i++)
            {
